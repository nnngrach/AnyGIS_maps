...
Однако многие карты время от времени обновляются, становясь лучше и точнее. К сожалению, по умолчанию Локус запрещает всем хранящимся в кэше картам скачиваться. Так что следует вручную произвести дополнительные настройки для всех карт, которые вы хотите обновлять. Для этого:
...
Для всех растровых ("отсканированных") карт время хранение кэша можно оставить без изменений, то есть бесконечным. Врядли их кто-то уже будет перерисовывать. Так что можно пропускать все оставшиеся карты из категории "Евразия". 
