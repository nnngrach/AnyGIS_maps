...
После этого Locus будет удалять из кэша всё, что не находится в данный момент на экране.

Если же это по какой-то причине не стработает, то следует еще раз открыть это же меню, но на этот раз выберать пункт `Очистка кэша`. Сохраненные данные удалятся и карта загрузится заново.
