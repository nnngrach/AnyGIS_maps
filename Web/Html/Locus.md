...
* Откройте меню карт
* Нажмите на кнопку с тремя точками справа от заголовка нужной карты.
* Выберие пункт `Установить Срок действия`
* Укажите минимально возможное значение: 0. 

### Настройка кэша
После того, как карты загрузятся и появятся в Локусе, будет полезно настроить время их хранения в кэше. Напомню, что все фрагменты карты, прежде, чем отобразиться на экране, скачиваются в память смартфона. Благодаря этому все сохраненные области могут отображаться и без интернета.

Но многие карты время от времени обновляются, становятся лучше и точнее. К сожалению, по умолчанию Локус запрещает скачанным картам обновляться. Значит, следует вручную произвести дополнительные настройки:

* Откройте меню карт
* Нажмите на кнопку с тремя точками справа от заголовка какой-либо карты.
* Выберие пункт `Установить "Срок действия"`
* Укажите, сколько через сколько часов можно обновлять выбранную карту.
...
Для всех динамически изменяющихся карт. То есть для всех карт и оверлеев с пробками: Яндекс Пробки, Google Traffic, Here Traffic, OpenSeaMap Traffic.
...
Для всех спутниковые снимков. 

##### 8760 часов (365 дней)

Для редко обновляющихся, но все же обновляющихся карт. Это Карты Завьялова (ММБ), Хребтовки Завьялова, карты для споритвное ориентирования Adraces и OSport.

##### По умолчанию (Бесконечно)

Для растровых ("отсканированных") время хранение кэша можно оставить без изменений, то есть бесконечным. Врядли их кто-то будет перерисовывать. Так что можно пропускать все оставшиеся карты из категории "Евразия". 
