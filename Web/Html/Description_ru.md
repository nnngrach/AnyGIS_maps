...
Некоторые сайты с картами отображают информацию только для авторизированных пользователей. Например, известная многим Strava Heatmap. Если просматривать  эту карту через браузер компьютера или смартфона, то никаких проблем не возникает: вводим логин и пароль, жмем на кнопку войти и сайт показывает карту. 
...
Когда навигатор подключится к нашему серверу, сервер зайдет на сайт с картой, введет логин и пароль, загрузит требуемое изображение и затем перешлет его обратно навигатору. Как видите, все довольно просто. Хотя и требует дополнительного времени на работу этого промежуточного компонента. Более подробно о технической реализации данного скрипта вы можете прочитать в моей статье на [Habr.com][3].
