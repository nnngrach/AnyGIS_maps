...
* Поместите скачанный файл карты с расширением `.sqlitedb` на свой смартфон в папку по умолчанию  `Android\data\net.osmand\files\tiles` 
* Если по каким-то причинам у вас нет такой папки (например, Osmand установлен на SD-карту), то нужно узнать, где именно находится директория программы. Для этого запустите приложение, откройте `Настройки`, выберите пункт `Основные настройки` и промотайте вниз до пункта `Каталог для хранения данных`. Здесь будет указан адрес. Карты нужно копировать в подпапку `tiles`
